        ...
        class Table extends React.Component {
            ...
            config() {
                return {
                    ...
                    peopelUrl: 'https://swapi.dev/api/people',
            ...
            filter(filter){
                ...
                let obj = this.state.baseData;
                let filtred = obj.filter( (el) =>{
                    return el.name.indexOf(val) >= 0
                });
                this.setState({ results: filtred } );
            }

            getPeopleInfo() {
                const url = this.config().peopelUrl;
                ...
                fetch(url, options)
                    ...
                    .then( (data ) => {
                        ...
                        this.setState({baseData: data.results} );
                    });
            }

            showPlanetDetails(evt){
                const url = this.config().planetUrl + evt.target.id;
                ...
                fetch(url, options)
                    ...
                    .then( (data ) => {
                        ...
                        this.setState({planetRender: true} );
                    });
            }
            ...
            renderTableData() {
                ...
                return this.state.results.map((person, index) => {
                        idWorld = person.homeworld.split('/').reverse()[1];
                    return (
                        <tr key={index + 1}>
                            <td>{person.name}</td>
                            <td>{person.height}</td>
            ...
            render() {
                return (
                    <div>
                        ...
                        <div className="row">
                            <div className="col-sm-1">Search Name: </div>
                            <div className="col-sm-10">
                                <input type="text" id="filterName" name="Search Name" placeholder="search" defaultValue="" onChange={ (filter) => this.filter(filter)}/>
                            </div>
                        </div>
                        <Popup planet={ this.state.planetDetails } render={ this.state.planetRender }/>
                    </div>
                )
            }
